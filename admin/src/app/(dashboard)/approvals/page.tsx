...
export default function ApprovalsPage() {
  ...
  return (
    <div className="p-8 text-foreground">
          ...
          <button
            ...
            className={`px-5 py-2.5 text-sm font-medium rounded-t-lg transition-colors border-b-2 -mb-px ${
              activeTab === tab
                ...
                : 'border-transparent text-foreground/70 hover:text-foreground hover:bg-surface-alt/50'
            }`}
          >
            {tab}
      ...
      {isLoading ? (
        <div className="text-foreground/70 py-12">Loading doctors...</div>
      ) : doctors.length === 0 ? (
        <div className="bg-surface border border-border rounded-2xl p-12 text-center text-foreground/70">
        ...
        <div className="grid gap-6">
          {doctors.map((doctor) => (
            <div key={doctor.doctorId} className="bg-surface border border-border rounded-2xl p-6 flex flex-col md:flex-row gap-6">
              <div className="flex-1">
                <div className="flex flex-wrap items-center gap-2 mb-2">
                  <h3 className="text-xl font-bold text-foreground">{doctor.fullName}</h3>
                  <span className={`inline-flex px-2 py-0.5 rounded text-xs font-medium border ${statusBadgeClass[doctor.status] ?? 'bg-surface-alt text-foreground/70 border-gray-600'}`}>
                    {statusLabel[doctor.status] ?? doctor.status}
                  </span>
                  <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium border ${doctor.isActive ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-surface-alt text-foreground/70 border-gray-600'}`}>
                    <span className={`w-1.5 h-1.5 rounded-full mr-1.5 ${doctor.isActive ? 'bg-emerald-400' : 'bg-gray-500'}`} />
                    {doctor.isActive ? 'Active' : 'Inactive'}
                  </span>
                </div>
                <p className="text-foreground/70 mb-2">{doctor.specialty} · {doctor.email}</p>
                {doctor.rejectionReason && (
                  <p className="text-red-400 text-sm mt-1">Rejection reason: {doctor.rejectionReason}</p>
                )}
              </div>

              <div className="flex flex-col gap-3 min-w-[200px] border-t md:border-t-0 md:border-l border-border pt-4 md:pt-0 md:pl-6">
                {/* Documents button — always shown */}
                <button
                  onClick={() => setDrawerUserId(doctor.userId)}
                  className="w-full bg-surface-alt hover:bg-surface-alt text-foreground/90 border border-border font-medium py-2 px-4 rounded-xl transition-all flex items-center justify-center gap-2"
                >
                ...
                {doctor.status === 'PendingReview' && (
                  <>
                    <button
                      ...
                      className="w-full bg-emerald-500/10 text-emerald-500 hover:bg-emerald-500 hover:text-foreground border border-emerald-500/20 font-medium py-2 px-4 rounded-xl transition-all flex items-center justify-center"
                    >
                    ...
                    {selectedDoctorId === doctor.doctorId ? (
                      <div className="space-y-3">
                        <textarea
                          ...
                          className="w-full bg-surface-alt border border-border rounded-lg p-2 text-sm text-foreground focus:outline-none focus:border-red-500 h-20 resize-none"
                        />
                        <div className="flex gap-2">
                          <button
                            onClick={() => handleReject(doctor.doctorId)}
                            className="flex-1 bg-red-500 text-foreground font-medium py-2 rounded-lg text-sm hover:bg-red-600 transition-colors"
                          >
                            Confirm
                          </button>
                          <button
                            onClick={() => { setSelectedDoctorId(null); setRejectionReason(''); }}
                            className="flex-1 bg-surface-alt text-foreground font-medium py-2 rounded-lg text-sm hover:bg-surface-alt/80 transition-colors"
                          >
                            Cancel
                          </button>
                        </div>
                      ...
                      <button
                        ...
                        className="w-full bg-red-500/10 text-red-500 hover:bg-red-500 hover:text-foreground border border-red-500/20 font-medium py-2 px-4 rounded-xl transition-all flex items-center justify-center"
                      >
                ...
                {doctor.status === 'Rejected' && (
                  <>
                    ...
                    <button
                      ...
                      className="w-full bg-yellow-500/10 text-yellow-500 hover:bg-yellow-500 hover:text-foreground border border-yellow-500/20 font-medium py-2 px-4 rounded-xl transition-all flex items-center justify-center mt-2"
                    >
                      Set to Pending
                    </button>
                  </>
                ...
                {(doctor.status === 'Approved' || doctor.status === 'Rejected') && (
                  <button
                    ...
                    className={`w-full font-medium py-2 px-4 rounded-xl transition-all flex items-center justify-center gap-2 border disabled:opacity-50 disabled:cursor-not-allowed ${
                      doctor.isActive
                        ? 'bg-red-500/10 text-red-400 hover:bg-red-500 hover:text-foreground border-red-500/20'
                        : 'bg-emerald-500/10 text-emerald-400 hover:bg-emerald-500 hover:text-foreground border-emerald-500/20'
                    }`}
                  >
                ...
                <div className="mt-4 pt-4 border-t border-border">
                  {deletingDoctorId === doctor.doctorId ? (
                    <div className="space-y-3">
                      ...
                      <div className="flex gap-2">
                        <button
                          ...
                          className="flex-1 bg-red-600 text-foreground font-medium py-2 rounded-lg text-sm hover:bg-red-700 transition-colors"
                        >
                          Confirm Delete
                        </button>
                        <button
                    ...
                    <button
                      ...
                      className="w-full bg-red-900/20 text-red-500 hover:bg-red-600 hover:text-foreground border border-red-900/30 font-medium py-2 px-4 rounded-xl transition-all flex items-center justify-center"
                    >
                      Delete Account
                    </button>
                  )}
